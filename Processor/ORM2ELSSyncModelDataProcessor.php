...
class ORM2ELSSyncModelDataProcessor {
    ...
    public function sync($modelId = null) {
        ...
        if ($modelId) {
            if ($item = $ormModel::find($modelId)) {
                $this->sync_item($this->elasticModel, $item, $keyName);
            }
        } else {
            ...
            $elasticModel::extendIndexingQueryChunk($builder, 1000, function($collection) use ($keyName) {
                $keyValues = [];
